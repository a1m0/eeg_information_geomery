...
# تحلیل اطلاعات هندسی حالات شناختی در سیگنال‌های EEG - مطالعه‌ای درباره فاصله‌های احتمالی و تعمیم‌پذیری بین‌سوژه‌ای
## چکیده

سیگنال‌های الکتروانسفالوگرافی (EEG) نمای ارزشمندی از فعالیت مغز ارائه می‌دهند، اما استخراج اطلاعات قابل اعتماد درباره حالات شناختی از این سیگنال‌ها چالش برانگیز است. این مقاله روش نوآورانه‌ای برای کمی‌سازی تفاوت‌های احتمالی بین حالات شناختی مختلف با استفاده از مفاهیم هندسه اطلاعات پیشنهاد می‌دهد. بر روی مجموعه داده DEAP (۳۲ فرد، ۱۲۸۰ نمونه)، ما معیارهای متعددی برای اندازه‌گیری فاصله بین توزیع‌های احتمالی را پیاده‌سازی کردیم، ارزیابی تقاطع بین‌سوژه‌ای را انجام دادیم، و معنادار بودن آماری جدایی را آزمایش کردیم. نتایج تقاطع میانگین دقت ۷۷.۷٪ (± ۱۱.۶٪) را نشان می‌دهند، که بالاتر از سطح تصادفی است؛ با این حال، آزمایش جایگشتی (p = ۰.۸۰۳) معنی‌داری آماری جدایی حالات را تأیید نمی‌کند. این نتایج نشان‌دهنده آن است که در حالی‌که روش ما بخش‌های معقول از تنوع را دریافت می‌کند، برای ادعای جدایی معنادار احتمالاً نیاز به بهبود ویژگی‌ها یا داده‌های بیشتر است.

**کلیدواژه‌ها**: الکتروانسفالوگرافی (EEG)، هندسه اطلاعات، تحلیل احتمالی، فاصله Jensen-Shannon، تقاطع بین‌سوژه‌ای، آزمایش جایگشتی
...
4. **شفافیت و تولید مثل**: مستندات کامل و کد قابل تولید مثل برای تسهیل کار محققان آتی
...
تحلیل سیگنال‌های EEG برای تفریق حالات شناختی پزشکی، علوم مغز و کاربردهای تعاملی انسان-کامپیوتر در تاریخ را داراست. روش‌های کلاسیک برای استخراج ویژگی شامل تحلیل طیفی (بر پایه تبدیل فوریه)، تحلیل فرکانس بیشتر و موجک است. توزیع قدرت طیفی در باند‌های شناخته‌شده (delta، theta، alpha، beta، gamma) معمولاً منعکس کننده حالات فعال‌سازی مختلف مغز تلقی می‌شود.
...
تقاطع‌سازی روش استاندارد برای ارزیابی توانایی تعمیم مدل‌هایی است. Leave-Subject-Out (LOO) variant بخصوص برای داده‌های EEG مرتبط به انسان مناسب است، زیرا انتظار داریم کاربرد واقعی روی سوژه‌های جدید صورت گیرد. روشی حیاتی در LOSOCV تفاوت میان "data leakage" و "proper generalization" است - وقتی تقلیل ابعاد برای تمام داده‌ها قبل از تقسیم به تقاطع انجام می‌شود، نتایج تقاطع متحیز خواهد بود.
...
**چرا Permutation Test؟** این روش غیرپارامتری شامل و قطعاً معتبری برای آزمایش معنی‌داری است.
...
### ۴.۴ نتایج تقاطع (Cross-Validation)
...
1. **دقت بالا می‌تواند صرفاً تصادفی باشد**: اگر حالات بسیار متفاوت هستند (آستانه شدید) اما بر اساس راز نویز، مدل می‌تواند نویز را بیاموزد.
...
3. **Overfitting درون‌ فرض‌ها**: توزیع‌های برازش‌شده ممکن است بیش‌از حد برای داده‌های مشاهده‌شده شیاق باشند.
...
2. ✓ **تولید مثل**: کد کامل و قابل اجرا تفراهم‌شده است
...
- EEG نویز بسیار است و هر ۷۷.۷٪ دقت می‌تواند اساساً نویز مرتبط با نویز باشد
- فیچرهای استخراج‌شده (قدرت طیفی) ممکن است برای تفریق دقیق valence کافی نباشند
